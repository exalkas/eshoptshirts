...
import { getDepartments,addDepartment } from '../../../actions/products_actions';
...
class ManageDepartments extends Component {
    ...
    updateForm = (element) => {
        const newFormdata = update(element,this.state.formdata,'departments');
        this.setState({
            formError: false,
    ...
    submitForm= (event) =>{
        ...
        if(formIsValid){
            this.props.dispatch(addDepartment(dataToSubmit,existingWoods)).then(response=>{
...
export default connect(mapStateToProps)(ManageDepartments);
